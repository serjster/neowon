...
fn ladder_combo(
    ...
    ui.horizontal(|ui| {
        ...
        egui::ComboBox::from_id_salt(id)
            ...
            .show_ui(ui, |ui| {
                for &v in ladder {
                    if ui.selectable_label((*current - v).abs() < v * 1e-6, fmt_val(v)).clicked()
                    {
                        *current = v;
                        changed = true;
                    }
...
pub fn panel(
    ...
    egui::Panel::left("controls").default_size(285.0).show(&mut root, |ui| {
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            for ch in 0..2 {
                ui.collapsing(format!("Channel {}", ch + 1), |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ui.label("Coupling");
                        for (label, v) in
                            [("DC", Coupling::Dc), ("AC", Coupling::Ac), ("GND", Coupling::Gnd)]
                        {
                    ...
                    if ladder_combo(ui, &format!("probe{ch}"), "Probe", &mut probe, &PROBES, |v| {
                        format!("×{v}")
                    }) {
                        c.probe = probe;
                        dirty = true;
            ...
            ui.collapsing("Trigger", |ui| {
                ...
                ui.horizontal(|ui| {
                    ui.label("Source");
                    for src in 0..2 {
                        if ui.selectable_label(t.source == src, format!("CH{}", src + 1)).clicked()
                        {
                            t.source = src;
                            dirty = true;
                        }
                ...
                ui.horizontal(|ui| {
                    ui.label("Sweep");
                    for (label, s) in
                        [("Auto", Sweep::Auto), ("Normal", Sweep::Normal), ("Single", Sweep::Single)]
                    {
            ...
            ui.collapsing("Display", |ui| {
                ui.horizontal(|ui| {
                    ...
                    for p in Persistence::LADDER {
                        if ui.selectable_label(phosphor.persistence == p, p.label()).clicked() {
            ...
            ui.collapsing("Cursors", |ui| {
                ...
                if cur.time_on {
                    ...
                    ui.label(format!("Δt = {}   1/Δt = {}", fmt_si(dt, "s"), fmt_si(1.0 / dt.max(1e-12), "Hz")));
                }
                if cur.amp_on {
    ...
    egui::Panel::bottom("measure")
        ...
        .show(&mut root, |ui| {
            ui.horizontal(|ui| {
                ...
                for slot in 0..SLOTS {
                    if meas.latest[slot].is_some()
                        && ui
                            .selectable_label(meas.stats_slot == slot, SLOT_NAMES[slot])
                            .clicked()
                    {
            ...
            egui::ScrollArea::both().show(ui, |ui| {
                egui::Grid::new("meas-grid").striped(true).min_col_width(64.0).show(ui, |ui| {
                    ui.label("");
                    for slot in 0..SLOTS {
                        if meas.latest[slot].is_some() {
                            ui.label(egui::RichText::new(SLOT_NAMES[slot]).strong());
                        }
                    }
    ...
    if fft.enabled {
        egui::Window::new("Spectrum").default_width(680.0).show(&ctx, |ui| {
            ui.horizontal(|ui| {
                ui.label("Source");
                for slot in 0..SLOTS {
                    if ui.selectable_label(fft.source == slot, SLOT_NAMES[slot]).clicked() {
                        fft.source = slot;
                    }
                }
